...
(function ($, window, undefined) {
	...
	var resultFactory=function(element){
		return element.val+"("+element.meta+")";
	}
	...
	function Plugin(element, options) {
		...
		//this.expression = new  RegExp('(?:^|\\b|)' + this.options.token + '([\\S.]*)$');
		this.expression = new RegExp('(^|\\b|\\s)' + this.options.token + '([\u4e00-\u9fa5\\w.]*)$');
	...
	Plugin.prototype.select = function () {
		this.replace(this.options.resultFactory(this.filtered[this.index]));
	...
	Plugin.prototype.onItemClick = function (element, e) {
		...
		this.replace(this.options.resultFactory(element));
		this.$element.trigger('mention-selected',this.filtered[this.index]);
		this.hideList();
	};
